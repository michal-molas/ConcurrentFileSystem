...
bool is_path_valid(const char* path) {
    ...
    while (name_start < path + len) {
        char* name_end =
            strchr(name_start, '/');  // End of current path component, at '/'.
...
const char* split_path(const char* path, char* component) {
    const char* subpath =
        strchr(path + 1, '/');  // Pointer to second '/' character.
    if (!subpath)               // Path is "/".
        return NULL;
    if (component) {
...
char* make_path_to_parent(const char* path, char* component) {
    ...
    char* result =
        malloc(subpath_len + 1);  // Include terminating null character.
    strncpy(result, path, subpath_len);
    result[subpath_len] = '\0';

    if (component) {
        size_t component_len =
            len - subpath_len - 1;  // Skip final '/' as well.
...
char* make_map_contents_string(HashMap* map) {
    ...
    for (const char** key = keys; *key; ++key) result_size += strlen(*key) + 1;
...
size_t find_common_path(const char* path1, const char* path2,
                        char* common_path) {
    size_t i = 0;
    while (path1[i] != '\0' && path2[i] != '\0' && path1[i] == path2[i]) {
        common_path[i] = path1[i];
        i++;
    }
    common_path[i] = '\0';
    return i - 1;
